...
def interpolate_pos_embed1(model, state_dict, interpolation: str = 'bicubic', seq_dim=1):
    # Rescale the grid of position embeddings when loading from state_dict
    old_pos_embed = state_dict.get('vision_model.embeddings.position_embedding.weight', None)
    
    grid_size = round((model.vision_model.embeddings.position_embedding.weight.shape[0]) ** 0.5)
    # grid_size = round((model.embeddings.position_embedding.weight.shape[0]) ** 0.5)
    if old_pos_embed is None:
        return
    grid_size = to_2tuple(grid_size)
    extra_tokens = 0  # FIXME detect different token configs (ie no class token, or more)
    new_seq_len = grid_size[0] * grid_size[1] + extra_tokens
    if new_seq_len == old_pos_embed.shape[0]:
        return
...
def interpolate_pos_embed3(model, state_dict, interpolation: str = 'bicubic', seq_dim=1):
    # Rescale the grid of position embeddings when loading from state_dict
    old_pos_embed = state_dict.get('visual_encoder.attnpool.positional_embedding', None)
    
    grid_size = round((model.visual_encoder.attnpool.positional_embedding.shape[0]-1) ** 0.5)
    # grid_size = round((model.embeddings.position_embedding.weight.shape[0]) ** 0.5)
    if old_pos_embed is None:
        return
    grid_size = to_2tuple(grid_size)
    ...
    if pos_emb_tok is not None:
        ...
        new_pos_embed = pos_emb_img
    state_dict['visual_encoder.attnpool.positional_embedding'] = new_pos_embed
    
    

def interpolate_pos_embed2(model, state_dict, interpolation: str = 'bicubic', seq_dim=1):
    # Rescale the grid of position embeddings when loading from state_dict
    old_pos_embed = state_dict.get('visual_encoder.embeddings.position_embedding.weight', None)
    
    grid_size = round((model.visual_encoder.embeddings.position_embedding.weight.shape[0]) ** 0.5)
    # grid_size = round((model.embeddings.position_embedding.weight.shape[0]) ** 0.5)
    if old_pos_embed is None:
        return
    grid_size = to_2tuple(grid_size)
    ...
    if pos_emb_tok is not None:
        ...
        new_pos_embed = pos_emb_img
    state_dict['visual_encoder.embeddings.position_embedding.weight'] = new_pos_embed
    

def interpolate_query_tokens(state_dict, new_size, interpolation: str = 'bicubic', seq_dim=1):
    # Rescale the grid of position embeddings when loading from state_dict
    old_pos_embed = state_dict.get('query_tokens', None)    
    
    interpolated = F.interpolate(old_pos_embed.unsqueeze(0), size=new_size, mode=interpolation, align_corners=True).squeeze(0)
    print(f'interpolate query_tokens from {old_pos_embed.shape} to {interpolated.shape}')
